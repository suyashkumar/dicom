...
type Parser interface {
	// Parse DICOM data
	Parse(options ReadOptions) (*DataSet, error)
	// ParseNext reads and parses the next element
	ParseNext(options ReadOptions) *Element
}
...
func (p *parser) Parse(options ReadOptions) (*DataSet, error) {
...
func (p *parser) ParseNext(options ReadOptions) *Element {
...
func ReadDataSet(in io.Reader, bytes int64, options ReadOptions) (*DataSet, error) {
	...
	for buffer.Len() > 0 {
		...
		if buffer.Len() >= startLen { // Avoid silent infinite looping.
			panic(fmt.Sprintf("ReadElement failed to consume data: %decoder %decoder: %v", startLen, buffer.Len(), buffer.Error()))
		}
		if elem == endOfDataElement {
			// element is a pixel data and was dropped by options
			break
