...
func (r *reader) readNativeFrames(parsedData *Dataset, fc chan<- *frame.Frame, vl uint32) (pixelData *PixelDataInfo,
	...
	// Used to read whole frames at a time in readNativeFrame.
	bytesPerFrame := bytesAllocated * samplesPerPixel * pixelsPerFrame
	...
	// Allocating a buffer to hold a whole frame allows us to read the frame in one large chunk
	// which significantly improves performance compared to reading pixel by pixel.
	...
	for frameIdx := 0; frameIdx < nFrames; frameIdx++ {
		...
		if bitsAllocated == 1 {
			...
			case 8:
				currentFrame, _, err = readNativeFrame[uint8](bitsAllocated, MustGetInts(rows.Value)[0], MustGetInts(cols.Value)[0], bytesToRead, samplesPerPixel, pixelsPerFrame, frameBuf, r.rawReader)
			case 16:
				currentFrame, _, err = readNativeFrame[uint16](bitsAllocated, MustGetInts(rows.Value)[0], MustGetInts(cols.Value)[0], bytesToRead, samplesPerPixel, pixelsPerFrame, frameBuf, r.rawReader)
			case 32:
				currentFrame, _, err = readNativeFrame[uint32](bitsAllocated, MustGetInts(rows.Value)[0], MustGetInts(cols.Value)[0], bytesToRead, samplesPerPixel, pixelsPerFrame, frameBuf, r.rawReader)
			default:
...
func readNativeFrame[I constraints.Integer](bitsAllocated, rows, cols, bytesToRead, samplesPerPixel, pixelsPerFrame int, buf []byte, rawReader *dicomio.Reader) (frame.Frame, int, error) {
	...
	// Read the whole frame at once
	_, err := io.ReadFull(rawReader, buf)
	if err != nil {
	...
	// bytesAllocated is not needed as we use constants in switch
	bufIdx := 0
	...
	for pixel := 0; pixel < pixelsPerFrame; pixel++ {
		for value := 0; value < samplesPerPixel; value++ {
			...
			case 8:
				...
				bufIdx++
			case 16:
				v, ok := any(bo.Uint16(buf[bufIdx:])).(I)
				if !ok {
					return frame.Frame{}, bytesToRead, fmt.Errorf("internal error - readNativeFrame unexpectedly unable to type cast pixel buffer data to the I type (%T), where bitsAllocated=%v", *new(I), bitsAllocated)
				}
				nativeFrame.RawData[(pixel*samplesPerPixel)+value] = v
				bufIdx += 2
			case 32:
				v, ok := any(bo.Uint32(buf[bufIdx:])).(I)
				if !ok {
					return frame.Frame{}, bytesToRead, fmt.Errorf("internal error - readNativeFrame unexpectedly unable to type cast pixel buffer data to the I type (%T), where bitsAllocated=%v", *new(I), bitsAllocated)
				}
				nativeFrame.RawData[(pixel*samplesPerPixel)+value] = v
