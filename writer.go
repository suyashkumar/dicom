	...
	if elem.Tag == dicomtag.PixelData {
		...
		if elem.UndefinedLength {
			...
			for _, frame := range image.Frames {
				writeRawItem(e, frame.EncapsulatedFrame.Data)
			}
			...
			numPixels := len(image.Frames[0].NativeFrame.Data)
			numValues := len(image.Frames[0].NativeFrame.Data[0])
			length := numFrames * numPixels * numValues * image.Frames[0].NativeFrame.BitsPerSample / 8 // length in bytes
			...
			for frame := 0; frame < numFrames; frame++ {
				for pixel := 0; pixel < numPixels; pixel++ {
					for value := 0; value < numValues; value++ {
						if image.Frames[frame].NativeFrame.BitsPerSample == 8 {
							binary.Write(buf, binary.LittleEndian, uint8(image.Frames[frame].NativeFrame.Data[pixel][value])) //TODO: revisit little endian
						} else if image.Frames[frame].NativeFrame.BitsPerSample == 16 {
							binary.Write(buf, binary.LittleEndian, uint16(image.Frames[frame].NativeFrame.Data[pixel][value])) //TODO: revisit little endian
						}
					}
				}
			}
